...
export class EmployeesTableComponent implements OnInit {
  ...
  constructor(private http: HttpClient,
    private auth: Auth, private employeesRepository: EmployeesRepositoryService,
      ...
      if (accept) {
        ...
        this.selectedRows.forEach(x => {
          ...
          if (index > -1) {
            ...
            const employ = new Employee(x['name'], x['lastName'], x['id'], x['email'], x['phone']);
            this.employeesRepository.DeletePerson(employ);
            this.employeesRepository.GetAllEmployees().subscribe(data => this.data = data, error => console.log(error),
